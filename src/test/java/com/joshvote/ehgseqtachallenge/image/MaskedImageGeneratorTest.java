		...
		for (int x = 0; x < mask.getImageConfiguration().getWidth(); x++) {
        	for (int y = 0; y < mask.getImageConfiguration().getHeight(); y++) {
        		...
        		if (existing == null) {
        			...
        			assertNull(
    					String.format("We have a duplicate pixel value of (%d, %d, %d) at [%d, %d] and [%d, %d]", (rgb & 0x00ff0000) >> 16, (rgb & 0x0000ff00) >> 8, (rgb & 0x000000ff), existing.getX(), existing.getY(), src.getX(), src.getY()), 
    					existing
					);
        		}
        	}
